 ...
 * \param ctx PT context
 */
char *user_to_record(const struct user *user, const void *c)
{
	char *s;
	char buf[32];
	const struct pt_context *ctx = (const struct pt_context *)c;
	...
	if (user->paid1 && ((*user->paid1 == 'E' && ctx->protocol_version < PROTOCOL_VERSION_80) ||
	    (*user->paid1 == 'Y' && ctx->protocol_version > PROTOCOL_VERSION_51)))
		s = append_field(s, "paid1", "6");
	else s = append_field(s, "paid1", user->paid1 ? user->paid1 : "N");

	if (ctx->protocol_version >= PROTOCOL_VERSION_80)
		s = append_field(s, "ip", inet_ntoa(ctx->addr.sin_addr));
